...
int create_pdf( nwipe_context_t* ptr )
{
    ...
    snprintf( model_header, sizeof( model_header ), " %s: %s ", "Model", c->system_name );
    pdf_add_text_wrap( pdf, NULL, model_header, 14, 0, 755, PDF_BLACK, page_width, PDF_ALIGN_CENTER, &height );
    snprintf( serial_header, sizeof( serial_header ), " %s: %s ", "S/N", c->system_serial_no );
    ...
    snprintf( barcode, sizeof( barcode ), "%s:%s", c->system_name, c->system_serial_no );
    pdf_add_text_wrap(
