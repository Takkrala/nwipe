...
int create_pdf( nwipe_context_t* ptr )
{
    ...
    snprintf( model_header, sizeof( model_header ), " %s: %s ", "Device", nwipe_options.system_product_name );
    pdf_add_text_wrap( pdf, NULL, model_header, 14, 0, 755, PDF_BLACK, page_width, PDF_ALIGN_CENTER, &height );
    snprintf( serial_header, sizeof( serial_header ), " %s: %s ", "S/N", nwipe_options.system_serial_number );
    ...
    snprintf( barcode, sizeof( barcode ), "%s:%s", nwipe_options.system_product_name, nwipe_options.system_serial_number );
    pdf_add_text_wrap(
    ...
    snprintf( c->PDF_filename,
              ...
              "%s/nwipe_report_%s_System_%s_Serial_%s.pdf",
